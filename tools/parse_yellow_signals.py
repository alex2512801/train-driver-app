...
ВАЖНО: этот скрипт сам km+пк не проставляет — здесь только литеры сигналов и названия
станций/перегонов. Запусти после него `match_yellow_signals.py` (README, шаг 14) — он
дописывает в этот же файл поля from_km/from_pk/from_resolution (и to_*) по данным
`track_signals.json`/`track_stations.json` (профили пути, шаги 12-13). Резолвится пока
меньшинство записей (в основном участок Карымская — Чита и станционные горловины Карымской/
Чернышевска-Забайкальского вне зоны покрытия наших профилей) — смотри resolution у каждой
записи, а не только сам факт наличия from_km/to_km.
"""
import sys
import json
import openpyxl
