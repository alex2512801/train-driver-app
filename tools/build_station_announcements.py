...
Шилка — НЕ пробел, несмотря на то, что 1 из 6 строк "слишком далеко" (6200 м > 6000 м): она
дублирует ту же станцию, что и 2 других строки того же направления, которые УЖЕ успешно
нашли ту же самую точку т.2/т.1 (профиль печатает название станции по нескольку раз вдоль
протяжённости — та же картина, что у Карымской/Парка). Итоговый дедуп ниже (по станции +
направлению + источнику + найденной точке) убирает такие буквальные повторы результата —
объявление для Шилки уже звучит корректно, просто раньше в JSON лежало 2-3 одинаковых записи
вместо одной (без вреда для поведения — PointAnnouncementScheduler и так не различает
одинаковые по значению точки, — но лишнее в данных).

Итог — 69 объявлений (после дедупликации одинаковых по значению записей) из 85 строк
track_stations.json, остальные не резолвятся, оставлены без записи (не подставлено наугад).
"""
import json
import sys
...
def main():
    ...
    # Крупные станции печатают своё название на профиле по нескольку раз вдоль протяжённости
    # (см. Карымскую выше) — если несколько таких дублей-строк независимо порезолвились на ТУ
    # ЖЕ самую точку т.2/т.1 (как у Шилки: 2-3 строки на направление, все на один и тот же
    # сигнал), это не разные объявления, а один и тот же физический подъезд — оставляем одну
    # запись, а не N одинаковых.
    seen = set()
    deduped = []
    for r in results:
        key = (r['station'], r['direction'], r['source'], r['predvhodnoy_km'], r['predvhodnoy_pk'])
        if key in seen:
            continue
        seen.add(key)
        deduped.append(r)
