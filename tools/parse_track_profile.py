...
# название станции.
#
# Необязательный суффикс "-N"/"-NN" (\d{1,2}) — станции вида "Чита-1"/"Чита-2" (два разных
# раздельных пункта в одном городе), изначально пропущенные: `page.get_text('words')` даёт их
# ОДНИМ словом ("Чита-1", без пробела), но старый паттерн (только буквы+дефис) не пропускал
# цифру внутри — из-за этого Чита-1/Чита-2 ошибочно считались "вне зоны покрытия" (машинист
# заметил по присланной схеме станции Чита I, что она physически между Кадалой и Чита-2, то
# есть внутри уже разобранного профиля, а не на неразобранном участке).
STATION_PATTERN = re.compile(r'^[А-ЯЁ][а-яёА-ЯЁ-]{2,}(-\d{1,2})?$')
