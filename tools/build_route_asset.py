...
Что делает скрипт:
  ...
  3. Строит граф связности и находит кратчайший путь (по фактической длине) между узлом,
     ближайшим к Хилоку, и узлом, ближайшим к Чернышевску-Забайкальскому.
  4. Сохраняет упорядоченные точки пути в JSON.

ИСПРАВЛЕНО: раньше чернышевский конец находился как "самый восточный узел связной
компоненты" — это была ошибочная эвристика: bbox покрывает весь регион вплоть до границы,
и в связной компоненте нашёлся узел восточнее реального Чернышевска-Забайкальского (видимо,
другая ветка/станция дальше на восток), из-за чего собранная линия обрывалась не там и
последние ~50 км маршрута уходили в сторону от реального пути (подтверждено GPS-точкой
машиниста 52.501740, 117.004039 = 6585км 3пк, которая после сборки оказалась в 47.6 км
от построенной линии). Теперь конец ищется как узел, ближайший к [CHERNYSHEVSK_APPROX]
(этой же проверенной GPS-точке), так же, как начало ищется по [KHILOK_APPROX].
...
# Координата конца маршрута (Чернышевск-Забайкальский) — раньше вместо неё использовалась
# эвристика "самый восточный узел", которая оказалась неверной (см. докстринг модуля).
# Взята из проверенной GPS-точки машиниста: 52.501740, 117.004039 = 6585км 3пк.
CHERNYSHEVSK_APPROX = (117.004039, 52.501740)  # (lon, lat)
...
def main():
    ...
    end = min(main_component, key=lambda n: dist_m(n, CHERNYSHEVSK_APPROX))
