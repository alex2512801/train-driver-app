...
class RouteTrack(
    ...
    companion object {
        /**
         * Точка отсчёта реального маршрута: откалибровано по двум GPS-якорям машиниста
         * (GPS + реальный км+пк в одной точке), а не по чтению профиля пути на глаз:
         *   - 51.359412, 110.462016 → 5934км 7пк (156 м от начала линии OSM)
         *   - 51.940295, 113.619818 → 6213км 5пк (6.8 м от линии)
         * Оба якоря легли на линию с точностью до сотен метров — линейная модель
         * (старт + коэффициент × пикетаж по линии) точно проходит через обе точки.
         */
        const val HILOK_START_KM = 5934.65

        /**
         * Коэффициент растяжения линии, откалиброван по тем же двум якорям (см.
         * [HILOK_START_KM]) — решение системы из двух точек, а не прикидка по общей длине
         * участка: (6213.45 − 5934.65) / 279.724 ≈ 0.9967. Раньше здесь было 659/642.8 ≈ 1.025
         * (грубая оценка по разнице длин полилинии и профиля пути) — новое значение точнее.
         * Третий якорь ближе к Чернышевску-Забайкальскому пока не подтверждён (см. RouteTrack
         * doc) — при получении надёжной точки у того конца маршрута коэффициент стоит пересчитать.
         */
        const val HILOK_CHERNYSHEVSK_LENGTH_CORRECTION = 0.9966964753021659
